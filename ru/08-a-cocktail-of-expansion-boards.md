...
-----------------------
Печатная плата размером 3x3 см., представляет собой самостотельное устройство на ATtiny85, которое можно программировать через Pi Bridge ICSP Interconnect и Raspberry Pi настроенными на ней Gordon Henderson's Arduino IDE и расширениями для поддержки ATtiny. Схема позволяет следить за напряжением батареи (а также другое напряжение) и управление нагрузкой с высоким потреблением тока согласно правилам, запрограммированным через скетч Arduino. На плате есть 3 светодиода для индикации. Демонстрационный скетч на Arduino может использоваться как контролер насоса с пианием от сонечной батареи.  
