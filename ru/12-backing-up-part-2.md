...
После этого я выполнил `sudo raspi-config`, выбрал опцию *Expand root partition to fill SD card* и система расширила 4-х Гб раздел, что-бы заполнить свободное пространство на карточке. Фактическое изменение размера раздела осуществится при следующей загрузке.

Что-бы убедится, что это так, запустите команду `df`, которую не обязательно выполнять с правами root.

    $ df -h /
    Filesystem Size Used Avai l Use% Mounted on
    /dev/root 7.3G 1.6G 5.4G 23% /

Вывод выше показывает инфомрацию о моей корневой файловой системе с точкой монтирования `/`, она стала размером 7.3 Гб.
